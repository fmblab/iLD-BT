...
This repository contains the active-learning pipeline from the study. It applies
an iterative Design–Build–Test–Learn (LD-BT) workflow to engineer *Sr*UGT76G1 for
improved rebaudioside (RebA → RebD) production, then transfers the strategy to
UGTSL2. The three steps of a campaign (generate the design space, simulate
library-construction strategies, and train/validate the two-stage model) run as
parameterized notebooks over the curated data included here.
...
3. `execute_production.ipynb`: train and validate the two-stage model on collected DBTL data
...
Copyright (C) 2026 Dongjae Kim, Han Min Woo (Sungkyunkwan University).

This software is licensed under the GNU General Public License v3.0 (or, at your
